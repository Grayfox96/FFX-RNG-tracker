...
class FFXRNGTracker:
	...
	def __init__(self, damage_rolls_input: tuple[int, int, int, int, int, int]) -> None:
		...
		self.equipment_names = self.get_equipment_names('files/ffxhd-equipment-names.csv')
	...
	def get_equipment_names(self, equipment_names_file: str) -> dict[str, list[str]]:
		'''Retrieves the equipment names.'''
		equipment_names = {'weapon': [], 'armor': []}

		with open(get_resource_path(equipment_names_file)) as equipment_names_file_object:
			equipment_names_file_reader = csv.reader(equipment_names_file_object, delimiter=',')
			...
			for _ in range(66):
				equipment_names['weapon'].append(next(equipment_names_file_reader))
			...
			for _ in range(84):
				equipment_names['armor'].append(next(equipment_names_file_reader))
	...
	def _get_weapon_name(self, owner_index: int, abilities: list[int], slots: int) -> str:
		'''Returns a weapon's name given the owner, the abilities and the number of slots.'''
		# get number of certain ability types in the equipment
		elemental_strikes = len([a for a in (30, 34, 38, 42) if a in abilities])
		status_strikes = len([a for a in (46, 50, 54, 58, 62, 66, 70, 74) if a in abilities])
		status_touches = len([a for a in (47, 51, 55, 59, 63, 67, 71, 75) if a in abilities])
		...
		if 122 in abilities: # Capture
			...
			index = 2
		elif 15 in abilities and 19 in abilities and 17 in abilities: # Triple Overdrive, Triple AP and Overdrive → AP
			index = 3
		elif 15 in abilities and 17 in abilities: # Triple Overdrive and Overdrive → AP
			index = 4
		elif 14 in abilities and 18 in abilities: # Double Overdrive and Double AP
			index = 5
			...
			index = 15
		elif 6 in abilities and magic_bonuses == 3: # Magic Booster and three Magic +X%
			index = 16
			...
			index = 20
		elif 5 in abilities and (3 in abilities or 4 in abilities): # Magic Counter and Counter-Attack or Evade & Counter
			index = 21
		elif 3 in abilities or 4 in abilities: # Counter-Attack or Evade & Counter
			index = 22
			...
			index = 55
		elif strength_bonuses > 0 and magic_bonuses > 0: # Magic +X% and Strength +X%
			index = 56
		elif slots == 2 or slots == 3: # 2 or 3 slot weapon
			index = 57
		elif 104 in abilities or 105 in abilities: # Magic +10% or Magic +20%
			index = 58
		elif 100 in abilities or 101 in abilities: # Strength +10% or Strength +20%
			index = 59
	...
	def _get_armor_name(self, owner_index: int, abilities: list[int], slots: int) -> str:
		'''Returns an armor's name given the owner, the abilities and the number of slots.'''
		# get number of certain ability types in the equipment
		...
		status_proofs = len([a for a in (48, 52, 56, 60, 64, 68, 72, 76, 78, 80, 82) if a in abilities])
		...
		# check conditions for names in order of priority
		if 23 in abilities and 24 in abilities: # Break HP Limit and Break MP Limit
			index = 0
			...
			index = 5
		elif 84 in abilities and 85 in abilities and 88 in abilities and 87 in abilities: # Auto Shell, Auto Protect, Auto Reflect and Auto Regen
			index = 6
		elif 8 in abilities and 9 in abilities and 10 in abilities: # Auto-Potion, Auto Med and Auto Phoenix
			index = 7
			...
			index = 33
		elif 88 in abilities or 93 in abilities: # Auto Reflect or SOS Reflect
			index = 34
		elif 84 in abilities or 89 in abilities: # Auto Shell or SOS Shell
			index = 35
		elif 85 in abilities or 90 in abilities: # Auto Protect or SOS Protect
			index = 36
			...
			index = 40
		elif elemental_eaters + elemental_proofs > 2: # Any two elemental -proof or -eater of different elements
			index = 41
			...
			index = 65
		elif hp_bonuses == 2 and mp_bonuses == 2: # Any two HP +X% and any two MP +X%
			index = 66
		elif slots == 4: # Four slots
			index = 67
		elif defense_bonuses > 0 and magic_defense_bonuses > 0: # Defense +X% and Magic Defense +X%
			index = 68
			...
			index = 72
		elif 108 in abilities or 109 in abilities: # Defense +10% or Defense +20%
			index = 73
		elif 112 in abilities or 113 in abilities: # Magic Defense +10% or Magic Defense +20%
			index = 74
			...
			index = 77
		elif 106 in abilities or 107 in abilities: # Defense +3% or Defense +5%
			index = 78
		elif 110 in abilities or 111 in abilities: # Magic Defense +3% or Magic Defense +5%
			index = 79
	...
	def create_dropped_equipment(self, monster_name: str, killer_index: int, current_party_formation: dict[str, bool]) -> dict[str, Value]:
		...
		if number_of_abilities_max > 0:
			...
			for _ in range(number_of_abilities_max):
		...
		if equipment['type'] == 'weapon':
			equipment['name'] = self._get_weapon_name(owner_index, equipment['abilities_index'], number_of_slots)
		elif equipment['type'] == 'armor':
			equipment['name'] = self._get_armor_name(owner_index, equipment['abilities_index'], number_of_slots)
