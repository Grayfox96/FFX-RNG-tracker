...
def predict_equipment_drops(
        damage_rolls_input, abilities, characters, steals_range,
        piranha_kills_range, kilika_in_kills_range, kilika_out_kills_range):

    def check_equipment(
            monster_name, killer_index, kilika_in_kills=0,
            sinscale_kills=0, kilika_out_kills=0):

        monster_name = monster_name.lower()

        *_, equipment = rng_tracker.get_spoils(
            monster_name, killer_index, rng_tracker.current_party_formation)
            ...
            for ability in abilities:
                if ability in equipment['abilities']:
                    ...
                    scenario = {
                        'equipment': equipment,
                        'steals': steals,
                        'piranha_kills': piranha_kills - 4,
                        'kilika_in_kills': kilika_in_kills,
                        'sinscale_kills': sinscale_kills,
    ...
    for steals in range(steals_range[0], steals_range[1] + 1):
        ...
        for piranha_kills in range(
                piranha_kills_range[0] + 4, piranha_kills_range[1] + 4 + 1):
            ...
            for kilika_enemy in ('killer_bee', 'dinonix', 'yellow_element'):
                ...
                if kilika_enemy == 'killer_bee':
                    kilika_killers = (4, 8)
                elif kilika_enemy == 'dinonix':
                    kilika_killers = (0, 4, 8)
                elif kilika_enemy == 'yellow_element':
                    kilika_killers = (8,)
                else:
                    kilika_killers = (0, 4, 8)
                ...
                for kilika_killer in kilika_killers:
                    ...
                    # check for the different amounts of sinscale kills in the
                    # echuilles fight
                    for sinscale_kills in (2, 4):

                        # check for every possible number of kills before
                        # geneaux
                        for kilika_in_kills in range(
                                kilika_in_kills_range[0],
                                kilika_in_kills_range[1] + 1):

                            # check for every possible number of kills after
                            # geneaux
                            for kilika_out_kills in range(
                                    kilika_out_kills_range[0],
                                    kilika_out_kills_range[1] + 1):

                                # reset the rng positions
                                rng_tracker.reset_variables()

                                # 15 kills before klikk
                                rng_tracker.rng_current_positions[10] += (
                                    15 + piranha_kills) * 3
                                ...
                                for enemy, killer_index in boat_forced_kills:
                                    check_equipment(enemy, killer_index)

                                for _ in range(kilika_in_kills):
                                    check_equipment(
                                        kilika_enemy, kilika_killer,
                                        kilika_in_kills)

                                kilika_forced_kills = (
                                    ('geneaux\'s_tentacle', 8),
                                    ('geneaux\'s_tentacle', 8),
                                    ('sinspawn_geneaux', 8),
                                )
                                ...
                                for enemy, killer_index in kilika_forced_kills:
                                    check_equipment(
                                        enemy, killer_index, kilika_in_kills)
                                ...
                                for enemy, killer_index in luca_forced_kills:
                                    check_equipment(
                                        enemy, killer_index, kilika_in_kills,
                                        sinscale_kills, kilika_out_kills)
...
if __name__ == '__main__':
    ...
    print(f'Testing all scenarios with '
          ...
          f'for weapon drops for {"/".join(characters)} '
          f'in Besaid, Kilika and Luca '
          f'with at least 1 of these abilities: {", ".join(abilities)}')

    print('(Assuming no deaths and no optional Ragora kills in Kilika)')

    good_equipments_scenarios = predict_equipment_drops(
        damage_rolls_input,
        abilities,
        characters,
        steals,
    ...
    if good_equipments_scenarios:
        output = (f'{"".join(["-" for i in range(120)])}\n'
                  'Steals | Piranhas | Sinscales | Kilika In | Kilika Out | '
                  '         Enemy |  Killer | Owner | Abilities\n'
        ...
        for scenario in good_equipments_scenarios:
            ...
            if scenario['equipment']['killer_index'] == 0:
                ...
                scenario['killer'] = 'Yuna'
            elif scenario['equipment']['killer_index'] == 2:
                scenario['killer'] = 'Auron'
            elif scenario['equipment']['killer_index'] == 3:
                scenario['killer'] = 'Kimahri'
            elif scenario['equipment']['killer_index'] == 4:
                scenario['killer'] = 'Wakka'
            elif scenario['equipment']['killer_index'] == 5:
                scenario['killer'] = 'Lulu'
            elif scenario['equipment']['killer_index'] == 6:
                scenario['killer'] = 'Rikku'
            elif scenario['equipment']['killer_index'] == 8:
                scenario['killer'] = 'Valefor'
            else:
                scenario['killer'] = '???'

            output += (f'{scenario["steals"]:>6} | '
                       f'{scenario["piranha_kills"]:>8} | '
                       f'{scenario["sinscale_kills"]:>9} | '
                       f'{scenario["kilika_in_kills"]:>9} | '
                       f'{scenario["kilika_out_kills"]:>10} | '
                       f'{scenario["equipment"]["monster_name"]:>14} | '
                       f'{scenario["killer"]:>7} | '
                       f'{scenario["equipment"]["owner"]:>5} | '
                       f'{", ".join(scenario["equipment"]["abilities"])}\n')

        output += ''.join(['-' for i in range(120)])
        print(output)
